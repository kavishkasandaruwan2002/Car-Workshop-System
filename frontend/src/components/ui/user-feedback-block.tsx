...
type FeedbackType = (typeof FEEDBACK_TYPES)[number]["type"];
            ...
            {feedbackType === "star" && (
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    key={n}
                    className={`text-yellow-400 ${n <= rating ? "" : "opacity-30"}`}
                    onClick={() => setRating(n)}
                    aria-label={`Rate ${n} star${n > 1 ? "s" : ""}`}
                  >
                    <Star className="w-6 h-6" fill={n <= rating ? "#facc15" : "none"} />
                  </button>
                ))}
              </div>
            )}
            {feedbackType === "emoji" && (
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    key={em.e}
                    className={`text-2xl ${emoji === em.e ? "ring-2 ring-primary rounded-full" : ""}`}
              ...
              <div className="flex gap-2">
                <button
                  className={`p-2 rounded-full border ${
                    thumb === "up" ? "bg-green-100 border-green-400" : "border-muted-foreground/20"
                  }`}
                  onClick={() => setThumb("up")}
                ...
                <button
                  className={`p-2 rounded-full border ${
                    thumb === "down" ? "bg-red-100 border-red-400" : "border-muted-foreground/20"
                  }`}
