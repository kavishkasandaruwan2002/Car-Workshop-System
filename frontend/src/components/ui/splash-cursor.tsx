...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const blit = (() => {
      ...
      const aPos = gl.getAttribLocation((copyShader as any) && (copyShader as any).program ? (copyShader as any).program : ({} as any), "aPosition");
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: any) {
      ...
      gl.uniform1f(
        ...
        (canvas.width as number) / (canvas.height as number)
      );
    ...
    function correctRadius(radius: number) {
      let aspectRatio = (canvas.width as number) / (canvas.height as number);
    ...
    function updatePointerDownData(pointer: any, id: number, posX: number, posY: number) {
      ...
      pointer.down = true;
      pointer.moved = false;
      pointer.texcoordX = posX / (canvas.width as number);
      pointer.texcoordY = 1.0 - posY / (canvas.height as number);
    ...
    document.body.addEventListener(
      ...
      function handleFirstMouseMove(e) {
        let pointer = pointers[0];
        let posX = scaleByPixelRatio((e as MouseEvent).clientX);
        let posY = scaleByPixelRatio((e as MouseEvent).clientY);
